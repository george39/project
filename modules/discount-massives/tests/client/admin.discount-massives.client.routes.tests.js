(function () {
  'use strict';

  describe('DiscountMassives Route Tests', function () {
    // Initialize global variables
    var $scope;
    var DiscountMassivesService;

    // We can start by loading the main application module
    beforeEach(module(ApplicationConfiguration.applicationModuleName));

    // The injector ignores leading and trailing underscores here (i.e. _$httpBackend_).
    // This allows us to inject a service but then attach it to a variable
    // with the same name as the service.
    beforeEach(inject(function ($rootScope, _DiscountMassivesService_) {
      // Set a new global scope
      $scope = $rootScope.$new();
      DiscountMassivesService = _DiscountMassivesService_;
    }));

    describe('Route Config', function () {
      describe('Main Route', function () {
        var mainstate;
        beforeEach(inject(function ($state) {
          mainstate = $state.get('admin.discountMassives');
        }));

        it('Should have the correct URL', function () {
          expect(mainstate.url).toEqual('/discountMassives');
        });

        it('Should be abstract', function () {
          expect(mainstate.abstract).toBe(true);
        });

        it('Should have template', function () {
          expect(mainstate.template).toBe('<ui-view/>');
        });
      });

      describe('List Route', function () {
        var liststate;
        beforeEach(inject(function ($state) {
          liststate = $state.get('admin.discountMassives.list');
        }));

        it('Should have the correct URL', function () {
          expect(liststate.url).toEqual('');
        });

        it('Should be not abstract', function () {
          expect(liststate.abstract).toBe(undefined);
        });

        it('Should have templateUrl', function () {
          expect(liststate.templateUrl).toBe(
            '/modules/discountMassives/client/views/admin/list-discountMassives.client.view.html'
          );
        });
      });

      describe('Create Route', function () {
        var createstate;
        var DiscountMassivesAdminController;
        var mockDiscountMassive;

        beforeEach(inject(function ($controller, $state, $templateCache) {
          createstate = $state.get('admin.discountMassives.create');
          $templateCache.put(
            '/modules/discountMassives/client/views/admin/form-discountMassive.client.view.html',
            ''
          );

          // Create mock discountMassive
          mockDiscountMassive = new DiscountMassivesService();

          // Initialize Controller
          DiscountMassivesAdminController = $controller('DiscountMassivesAdminController as vm', {
            $scope: $scope,
            discountMassiveResolve: mockDiscountMassive
          });
        }));

        it('Should have the correct URL', function () {
          expect(createstate.url).toEqual('/create');
        });

        it('Should have a resolve function', function () {
          expect(typeof createstate.resolve).toEqual('object');
          expect(typeof createstate.resolve.discountMassiveResolve).toEqual('function');
        });

        it('should respond to URL', inject(function ($state) {
          expect($state.href(createstate)).toEqual('/admin/discountMassives/create');
        }));

        it('should attach an discountMassive to the controller scope', function () {
          expect($scope.vm.discountMassive._id).toBe(mockDiscountMassive._id);
          expect($scope.vm.discountMassive._id).toBe(undefined);
        });

        it('Should not be abstract', function () {
          expect(createstate.abstract).toBe(undefined);
        });

        it('Should have templateUrl', function () {
          expect(createstate.templateUrl).toBe(
            '/modules/discountMassives/client/views/admin/form-discountMassive.client.view.html'
          );
        });
      });

      describe('Edit Route', function () {
        var editstate;
        var DiscountMassivesAdminController;
        var mockDiscountMassive;

        beforeEach(inject(function ($controller, $state, $templateCache) {
          editstate = $state.get('admin.discountMassives.edit');
          $templateCache.put(
            '/modules/discountMassives/client/views/admin/form-discountMassive.client.view.html',
            ''
          );

          // Create mock discountMassive
          mockDiscountMassive = new DiscountMassivesService({
            _id: '525a8422f6d0f87f0e407a33',
            title: 'An DiscountMassive about MEAN',
            content: 'MEAN rocks!'
          });

          // Initialize Controller
          DiscountMassivesAdminController = $controller('DiscountMassivesAdminController as vm', {
            $scope: $scope,
            discountMassiveResolve: mockDiscountMassive
          });
        }));

        it('Should have the correct URL', function () {
          expect(editstate.url).toEqual('/:discountMassiveId/edit');
        });

        it('Should have a resolve function', function () {
          expect(typeof editstate.resolve).toEqual('object');
          expect(typeof editstate.resolve.discountMassiveResolve).toEqual('function');
        });

        it('should respond to URL', inject(function ($state) {
          expect(
            $state.href(editstate, {
              discountMassiveId: 1
            })
          ).toEqual('/admin/discountMassives/1/edit');
        }));

        it('should attach an discountMassive to the controller scope', function () {
          expect($scope.vm.discountMassive._id).toBe(mockDiscountMassive._id);
        });

        it('Should not be abstract', function () {
          expect(editstate.abstract).toBe(undefined);
        });

        it('Should have templateUrl', function () {
          expect(editstate.templateUrl).toBe(
            '/modules/discountMassives/client/views/admin/form-discountMassive.client.view.html'
          );
        });

        xit('Should go to unauthorized route', function () {});
      });
    });
  });
})();
