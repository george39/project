(function () {
  'use strict';

  describe('FeatureDetails Route Tests', function () {
    // Initialize global variables
    var $scope;
    var FeatureDetailsService;

    // We can start by loading the main application module
    beforeEach(module(ApplicationConfiguration.applicationModuleName));

    // The injector ignores leading and trailing underscores here (i.e. _$httpBackend_).
    // This allows us to inject a service but then attach it to a variable
    // with the same name as the service.
    beforeEach(inject(function ($rootScope, _FeatureDetailsService_) {
      // Set a new global scope
      $scope = $rootScope.$new();
      FeatureDetailsService = _FeatureDetailsService_;
    }));

    describe('Route Config', function () {
      describe('Main Route', function () {
        var mainstate;
        beforeEach(inject(function ($state) {
          mainstate = $state.get('admin.featureDetails');
        }));

        it('Should have the correct URL', function () {
          expect(mainstate.url).toEqual('/featureDetails');
        });

        it('Should be abstract', function () {
          expect(mainstate.abstract).toBe(true);
        });

        it('Should have template', function () {
          expect(mainstate.template).toBe('<ui-view/>');
        });
      });

      describe('List Route', function () {
        var liststate;
        beforeEach(inject(function ($state) {
          liststate = $state.get('admin.featureDetails.list');
        }));

        it('Should have the correct URL', function () {
          expect(liststate.url).toEqual('');
        });

        it('Should be not abstract', function () {
          expect(liststate.abstract).toBe(undefined);
        });

        it('Should have templateUrl', function () {
          expect(liststate.templateUrl).toBe(
            '/modules/featureDetails/client/views/admin/list-featureDetails.client.view.html'
          );
        });
      });

      describe('Create Route', function () {
        var createstate;
        var FeatureDetailsAdminController;
        var mockFeatureDetail;

        beforeEach(inject(function ($controller, $state, $templateCache) {
          createstate = $state.get('admin.featureDetails.create');
          $templateCache.put(
            '/modules/featureDetails/client/views/admin/form-featureDetail.client.view.html',
            ''
          );

          // Create mock featureDetail
          mockFeatureDetail = new FeatureDetailsService();

          // Initialize Controller
          FeatureDetailsAdminController = $controller('FeatureDetailsAdminController as vm', {
            $scope: $scope,
            featureDetailResolve: mockFeatureDetail
          });
        }));

        it('Should have the correct URL', function () {
          expect(createstate.url).toEqual('/create');
        });

        it('Should have a resolve function', function () {
          expect(typeof createstate.resolve).toEqual('object');
          expect(typeof createstate.resolve.featureDetailResolve).toEqual('function');
        });

        it('should respond to URL', inject(function ($state) {
          expect($state.href(createstate)).toEqual('/admin/featureDetails/create');
        }));

        it('should attach an featureDetail to the controller scope', function () {
          expect($scope.vm.featureDetail._id).toBe(mockFeatureDetail._id);
          expect($scope.vm.featureDetail._id).toBe(undefined);
        });

        it('Should not be abstract', function () {
          expect(createstate.abstract).toBe(undefined);
        });

        it('Should have templateUrl', function () {
          expect(createstate.templateUrl).toBe(
            '/modules/featureDetails/client/views/admin/form-featureDetail.client.view.html'
          );
        });
      });

      describe('Edit Route', function () {
        var editstate;
        var FeatureDetailsAdminController;
        var mockFeatureDetail;

        beforeEach(inject(function ($controller, $state, $templateCache) {
          editstate = $state.get('admin.featureDetails.edit');
          $templateCache.put(
            '/modules/featureDetails/client/views/admin/form-featureDetail.client.view.html',
            ''
          );

          // Create mock featureDetail
          mockFeatureDetail = new FeatureDetailsService({
            _id: '525a8422f6d0f87f0e407a33',
            title: 'An FeatureDetail about MEAN',
            content: 'MEAN rocks!'
          });

          // Initialize Controller
          FeatureDetailsAdminController = $controller('FeatureDetailsAdminController as vm', {
            $scope: $scope,
            featureDetailResolve: mockFeatureDetail
          });
        }));

        it('Should have the correct URL', function () {
          expect(editstate.url).toEqual('/:featureDetailId/edit');
        });

        it('Should have a resolve function', function () {
          expect(typeof editstate.resolve).toEqual('object');
          expect(typeof editstate.resolve.featureDetailResolve).toEqual('function');
        });

        it('should respond to URL', inject(function ($state) {
          expect(
            $state.href(editstate, {
              featureDetailId: 1
            })
          ).toEqual('/admin/featureDetails/1/edit');
        }));

        it('should attach an featureDetail to the controller scope', function () {
          expect($scope.vm.featureDetail._id).toBe(mockFeatureDetail._id);
        });

        it('Should not be abstract', function () {
          expect(editstate.abstract).toBe(undefined);
        });

        it('Should have templateUrl', function () {
          expect(editstate.templateUrl).toBe(
            '/modules/featureDetails/client/views/admin/form-featureDetail.client.view.html'
          );
        });

        xit('Should go to unauthorized route', function () {});
      });
    });
  });
})();
