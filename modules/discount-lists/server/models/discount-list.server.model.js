...
/**
 * Discount List Schema
 */
var DiscountListSchema = new Schema({
  name: {
    type: String
  },
  status: {
...
// methods
DiscountListSchema.methods.processFilter = function (params) {
  ...
  for (var field in DiscountListSchema.paths) {
    if (DiscountListSchema.paths.hasOwnProperty(field)) {
      continue;
    }
    ...
    if (DiscountListSchema.paths[field].instance === 'Number') {
...
DiscountListSchema.methods.processPopulate = function (params) {
  if (!params || typeof params == 'undefined') {
    return {
      path: '',
      select: ''
...
DiscountListSchema.methods.processSort = function (params) {
  if (!params || typeof params == 'undefined') {
    return {
      modified: -1
    };
...
DiscountListSchema.statics.seed = seed;

mongoose.model('DiscountList', DiscountListSchema);

/**
 * Seeds the User collection with document (DiscountList)
  ...
  return new Promise(function (resolve, reject) {
    ...
    function add(skip) {
      return new Promise(function (resolve, reject) {
        if (skip) {
          return resolve({
            message: chalk.yellow('Database Seeding: DiscountList\t' + doc.title + ' skipped')
          });
        }

        var discountList = new DiscountList(doc);
        ...
        discountList.save(function (err) {
          ...
          return resolve({
            message: 'Database Seeding: DiscountList\t' + discountList.title + ' added'
          });
        });
      });
    }
