...
exports.signup = async function (req, res) {
  ...
  await Promise.all([
    Shop.findOne({ name: { $regex: 'Compusum', $options: 'i' } }),
    Group.findOne({ name: 'user' })
  ]).then((res) => {
    user.shop = res[0]._id;
    user.group = res[1]._id;
  });
 ...
 * Signup
 */
exports.signupAsShop = async function (req, res) {
  delete req.body.roles;
  ...
  user.group = managerGroup._id;

  Promise.all([user.save(), shop.save()])
    .then(() => {
      user.password = undefined;
      user.salt = undefined;
    ...
    .catch(() => {
      return res.status(422).json({ message: 'Ha ocurrido un error!' });
    });
};
