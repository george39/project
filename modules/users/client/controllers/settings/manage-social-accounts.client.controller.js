(function () {
  ...
  SocialAccountsController.$inject = [
    '$state',
    '$window',
    'UsersService',
    'Authentication',
  ...
  function SocialAccountsController($state, $window, UsersService, Authentication, Notification) {
    ...
    function isConnectedSocialAccount(provider) {
      return (
        vm.user.provider === provider ||
        (vm.user.additionalProvidersData && vm.user.additionalProvidersData[provider])
      );
    }
    ...
    function onRemoveSocialAccountSuccess(response) {
      ...
      Notification.success({ message: '<i class="far fa-thumbs-up"></i> Removed successfully!' });
    ...
    function onRemoveSocialAccountError(response) {
      Notification.warning({
        ...
        title: '<i class="fas fa-trash-alt"></i> Remove failed!'
      });
    }
