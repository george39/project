...
gulp.task('watch', function () {
  ...
  gulp
    .watch(defaultAssets.server.allJS, gulp.series('eslint'))
  ...
  gulp
    .watch(defaultAssets.client.sass, gulp.series('sass' /* , 'csslint'*/))
    .on('change', plugins.refresh.changed);
  gulp
    .watch(defaultAssets.client.less, gulp.series('less' /* , 'csslint'*/))
    .on('change', plugins.refresh.changed);

  if (process.env.NODE_ENV === 'production') {
    gulp.watch(defaultAssets.server.gulpConfig, gulp.series('templatecache', 'eslint'));
    gulp
      .watch(defaultAssets.client.views, gulp.series('templatecache'))
      .on('change', plugins.refresh.changed);
  } else {
...
gulp.task('watch:server:run-tests', function () {
  ...
  gulp
    .watch([testAssets.tests.server, defaultAssets.server.allJS], gulp.series('test:server'))
...
gulp.task(
  'mocha:coverage',
  gulp.series('pre-test', 'mocha', function () {
    var testSuites = changedTestFiles.length ? changedTestFiles : testAssets.tests.server;
...
gulp.task(
  'protractor',
  gulp.series('webdriver_update', function (done) {
...
gulp.task('lint', gulp.parallel('less', 'sass', gulp.series(/* 'csslint', */ 'eslint')));

// Lint project files and minify them into two production files.
gulp.task('build', gulp.series('wiredep:prod', 'lint', gulp.parallel('uglify', 'cssmin')));

gulp.task(
  'test:server',
  gulp.series(
    'env:test',
    gulp.parallel('copyLocalEnvConfig', 'makeUploadsDir', 'dropdb'),
    'lint',
    'mocha'
  )
);
...
gulp.task('test', gulp.series('env:test', 'test:server', 'karma'));
...
gulp.task(
  'default',
  gulp.series(
    gulp.parallel('copyLocalEnvConfig', 'makeUploadsDir'),
    'lint',
...
gulp.task(
  'prod',
  gulp.series(
    gulp.parallel('copyLocalEnvConfig', 'makeUploadsDir', 'templatecache'),
    'env:prod',
    'build',
    'lint',
    gulp.parallel('nodemon-nodebug', 'watch')
  )
);
